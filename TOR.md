...
## Указатели
[Архитектура монолит](#структура-проектамонолит)  
...
2. Хендлеры разбирают анмаршалят данные в [DTO](./internal/transport/http/dto/).
3. DTO передаются в сервисный слой, где происходит бизнес-логика.
4. Сервисный слой взаимодействует с репозиторием с помощью [доменных моделей](./internal/domain/), в которые конвертируются DTO объекты, поступающие от хендлеров.
5. Репозиторий выполняет запросы к БД или кэшу и возвращает нам доменные модели.
6. Сервисный слой обрабатывает полученные данные и возвращает результат обратно в хендлеры в виде DTO.   
### Итог:  
**DTO -> Service -> Domain Models -> Repository -> DB/Cache -> Repository -> Domain Models -> Service -> DTO**

**DTO** - объекты для передачи данных по сети(от клиента или клиенту).
**Domain Models** - бизнес модели, которые используются внутри приложения для работы с данными, или которые хранятся в репозиториях.

## Обработка данных
1. Входящие HTTP запросы обрабатываются в хендлерах, где данные анмаршалятся в DTO объекты.
2. Происходит базовая валидация DTO прямо в хендлере(например, проверка обязательных полей, или того что они не пустые), без применения бизнес-логики.
3. DTO объекты передаются в сервисный слой, где происходит основная бизнес-логика(валидация с бизнес-правилами, например проверка сложности пароля, его длины, возможности его обновления у пользователя и тд).
4. Репозиторий(слой доступа к данным) не занимается валидацией, он взаимодействует с базой данных или кэшем, используя доменные модели, которые уже валидны и корректны.
5. Ответы от репозитория возвращаются обратно в сервисный слой, где может происходить дополнительная обработка данных.